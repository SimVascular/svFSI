...
void FM_2WayNodeRefine(CtrlType *ctrl, GraphType *graph, float ubfactor, int npasses)
{
  ...
  for (pass=0; pass<npasses; pass++) {
    ...
    badmaxpwgt = (int)(ubfactor*(pwgts[0]+pwgts[1])/2);
...
void FM_2WayNodeRefine_OneSided(CtrlType *ctrl, GraphType *graph, float ubfactor, int npasses)
{
  ...
  for (pass=0; pass<npasses; pass++) {
        ...
        if (where[k] == 2) { /* For the in-separator vertices modify their edegree[to] */
          ...
          for (jj=xadj[k]; jj<xadj[k+1]; jj++) {
            ...
            if (where[kk] != 2) 
              ...
              /* Update the vertex if it has not been moved */
              if (inqueue[kk] == pass)
    ...
    *****************************************************************/
    for (nswaps--; nswaps>mincutorder; nswaps--) {
      if ((higain = swaps[nswaps]) == -1)
        continue; /* that was a skipped vertex */
...
void FM_2WayNodeBalance(CtrlType *ctrl, GraphType *graph, float ubfactor)
{
  ...
  /* badmaxpwgt = (pwgts[0]+pwgts[1]+pwgts[2])*ubfactor; */
  badmaxpwgt = (pwgts[0]+pwgts[1])*ubfactor;
  if (pwgts[0] <= badmaxpwgt && pwgts[1] <= badmaxpwgt)
    return;
  if (abs(pwgts[0]-pwgts[1]) < 3*(pwgts[0]+pwgts[1]+pwgts[2])/nvtxs)
    return;
